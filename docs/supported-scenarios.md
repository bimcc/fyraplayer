...
| PanoramaLite first-party WebGL2 renderer | Conditional, implementation in progress | `docs/panoramalite.md` defines the lightweight equirectangular image/video/live panorama plugin scope. API and core renderer pieces exist, but it must remain out of support claims until browser pixel evidence exists. |
