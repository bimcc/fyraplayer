...
| PanoramaLite first-party WebGL2 renderer | Verified for product-demo scope; hardening continues | `docs/panoramalite.md` defines the lightweight equirectangular image/video/live panorama plugin scope. Edge smoke evidence exists for generated image, MP4/file, public panorama HLS presets, live HLS, and live WebRTC. The main demo now has a unified ordinary/panorama source list and switches the visible control surface between the UI shell and PanoramaLite viewer controls at runtime. Source-platform metadata (`presentation.mode = 'panorama'`, `source.meta.presentation`, or `tags: ['panorama']`) can trigger this mode through `isPanoramaSource()` / `getSourcePresentation()`. Generated image panoramas are the zero-flip orientation baseline, image and video sources share zero-flip defaults, and quality-reducing caps are opt-in fallback knobs. `enabled` / `handle.setEnabled()` allows ordinary/panorama mode switching on the current video element when the plugin is installed. Optional in-view controls support fullscreen/touch usage. Current screen interaction writes yaw/pitch/fov only; programmatic roll remains available for future gyro/device-orientation and headset/WebXR work. Long-run WebGL resource sampling, controlled context restore, and deployment-specific orientation validation are still tracked as hardening items. |
