...
| PanoramaLite | P2 | done for product-demo scope; hardening continues | First-party lightweight WebGL2 equirectangular panorama renderer plugin for panoramic video, panoramic images, and live panorama playback. API, renderer, texture binding, controls, orientation correction, video upload throttling, canvas pixel caps, unit coverage, demo, and Edge smoke evidence exist. Keep it optional and dependency-free; see `docs/panoramalite.md` |
